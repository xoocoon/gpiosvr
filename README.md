...
More complete examples are included in the repo under `template/`:
...
A more complete example is included in the repo under `template/`:
...
Under `template/`, you will find systemd templates for installing `gpio_server.py` and `key_monitor.py` as systemd service units. If you do not want to use systemd as an init system, you can still use the `*.service` files as a reference for valid command lines.
